...
const Dialog = forwardRef<HTMLDialogElement, Props>(({ children, toggleDialog, type = "non-modal" }, ref) => {
  return (
    <dialog
      className={`rounded-3xl px-6 pb-6 pt-10 w-[360px] shadow-sm border border-gray-100 bottom-left ${type === "non-modal" ? "absolute bottom-8 ml-8" : ""}`}
      ref={ref}
      onClick={(e) => {
    ...
    >
      {children}
      <button className="absolute top-4 right-5 shadow-sm outline-none" onClick={toggleDialog}>
